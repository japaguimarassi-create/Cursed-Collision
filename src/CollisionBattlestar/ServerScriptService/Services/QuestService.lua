...
function S.Init(defeated:BindableEvent)defeated.Event:Connect(function(model:Model,uid:number)if uid<=0 or ((model:GetAttribute("BattleStreakOwnerUserId")or 0)>0) then return end;for _,p in Players:GetPlayers()do if p.UserId==uid then if Data.ProgressQuest(p,ID,1)then R.CollisionRemotes.Feedback:FireClient(p,"QuestComplete","FIRST RESPONSE COMPLETE • +50 CR")end;break end end end)end
