...
local function makeWave(wave:number):{string}
	...
	for i=1,count do local kind=pool[((i+wave-1)%#pool)+1];if wave%10==0 and i==count then kind="Boss"elseif wave%5==0 and i==count then kind="MiniBoss"end;table.insert(result,kind)end
	return result
end
local function spawnWave(p:Player)
	local s=sessions[p];if not s then return end
	s.Kinds=makeWave(s.Wave);s.Alive=0
	for i,kind in ipairs(s.Kinds)do local m=enemyService.Spawn(kind,arenaCenter+offsets[((i-1)%#offsets)+1]+Vector3.new(0,3,0),p.UserId);if m then s.Spawned[m]=true;s.Alive+=1;m:SetAttribute("BattleStreakWave",s.Wave)end end
