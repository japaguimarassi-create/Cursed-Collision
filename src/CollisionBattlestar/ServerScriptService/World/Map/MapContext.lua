...
function M.SetBossArena(pos:Vector3)M.Get().bossArena=pos end
function M.SetEventAnchor(pos:Vector3)M.Get().eventAnchor=pos end
function M.Destructible(parent:Instance,name:string,pos:Vector3,size:Vector3,color:Color3):Part
	local p=M.Part(parent,name,size,CFrame.new(pos),Enum.Material.Concrete,color,true,true,false)
	M.Tag(p,"CombatDestructible")
	p:SetAttribute("RestoreSeconds",6)
	local s=M.Get()
	if s.breakParts then table.insert(s.breakParts,p)end
	return p
end
return M
