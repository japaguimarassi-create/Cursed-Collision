--!strict
local CollectionService=game:GetService("CollectionService");local R=game:GetService("ReplicatedStorage");local C=require(R.Shared.Config);local U=require(R.Shared.Util);local Data=require(script.Parent.Parent.Services.PlayerDataService)
...
function S.Init()
	...
	local secret=U.Part(landmarks,"SecretDoor",Vector3.new(8,12,2),CFrame.new(0,6,-38),Enum.Material.Neon,Color3.fromRGB(85,255,195),true);CollectionService:AddTag(secret,"Interactable");local prompt=Instance.new("ProximityPrompt");prompt.ActionText="Inspect";prompt.ObjectText="Unknown Door";prompt.HoldDuration=.4;prompt.MaxActivationDistance=9;prompt.Parent=secret;prompt.Triggered:Connect(function(player)if player:GetAttribute("SecretDoorFound")~=true then player:SetAttribute("SecretDoorFound",true);Data.AddExploration(player,25);Data.AddCoins(player,25);game.ReplicatedStorage.CollisionRemotes.Feedback:FireClient(player,"SecretFound","Hidden discovery • +25 CR")end end)
